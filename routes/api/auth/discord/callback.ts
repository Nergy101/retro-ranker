...
export const handler: Handlers = {
  async GET(req, ctx) {
    return await tracer.startActiveSpan("discord-auth-callback", async (span) => {
      ...
      if (!state) {
        logJson("warn", "Missing state in Discord callback", { url: req.url });
      ...
      if (!codeVerifier) {
        logJson("warn", "Missing or invalid codeVerifier in Discord callback", { state });
      ...
      try {
        ...
        logJson("info", "Discord OAuth2 callback successful", { userId: user?.record?.id, state });
