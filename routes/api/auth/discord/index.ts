  ...
  async GET(req, ctx) {
    return await tracer.startActiveSpan("discord-auth-start", async (span) => {
      try {
        ...
        logJson("info", "Starting Discord OAuth2 flow", { state: randomId, url: discordUrl });
        ...
        logJson("error", "Error starting Discord OAuth2 flow", { error: error instanceof Error ? error.message : String(error) });
