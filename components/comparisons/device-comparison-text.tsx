...
export function DeviceComparisonText(
  ...
  const formatScreenSize = (size: number | null) => size ? `${size}-inch` : "N/A";
  const formatRam = (device: Device) =>
    device.ram?.sizes?.[0] ? `${device.ram.sizes[0]} ${device.ram.unit}` : "N/A";
  const formatBattery = (device: Device) =>
    device.battery?.capacity ? `${device.battery.capacity}${device.battery.unit}` : "N/A";
  const formatOS = (device: Device) =>
    device.os?.list?.length ? device.os.list.join(", ") : "Unknown OS";
  const formatPrice = (device: Device) =>
    device.pricing?.average ? `${device.pricing.average} ${device.pricing.currency}` : "N/A";
    ...
    <details class="device-comparison-details">
      ...
      <article class="device-comparison-text" style={{ marginTop: "1rem" }}>
        <p>
          ...
          are both retro gaming handhelds with different specifications and performance
          characteristics. This comparison highlights the key differences to help you choose the
          right device for your gaming needs.
        </p>

        <h3>Price Comparison</h3>
        <p>
          The <strong>{cheaperDevice.brand.raw} {cheaperDevice.name.raw}</strong>{" "}
          is priced at approximately <strong>{formatPrice(cheaperDevice)}</strong>, while the{" "}
          <strong>
          ...
          {priceDiff > 0
            ? (
              <>
                ...
                </strong>, making the {cheaperDevice.name.raw} the more budget-friendly option.
              </>
            )
        ...
        <p>
          ...
          <strong>{worseDevice.brand.raw} {worseDevice.name.raw}</strong>'s score of{" "}
          <strong>{formatRating(worseDevice.totalRating)}/10</strong>.
          {Number(perfDiff) > 0
            ? (
              <>
                {" "}This{" "}
                {perfDiff}-point difference reflects variations in processing power, emulation
        ...
        {(Number(screenDiff) > 0 || batteryDiff > 0 || ramDiff > 0) && (
          <p>
            {Number(screenDiff) > 0 && (
              <>
                The <strong>{biggerScreen.name.raw}</strong> has a {screenDiff}-inch larger screen.
              </>
            )}
            {batteryDiff > 0 && (
              <>
                The <strong>{betterBattery.name.raw}</strong> offers {batteryDiff}
                {device1.battery?.unit || "mAh"} more battery capacity.
              </>
            )}
            {ramDiff > 0 && (
              <>
                For memory, the <strong>{moreRam.name.raw}</strong> provides {ramDiff}{" "}
                {device1.ram?.unit || "GB"}{" "}
                more RAM, which can impact performance when running demanding emulators.
              </>
            )}
          </p>
        )}
        ...
        <p>
          ...
          {formatOS(device1)}, while the <strong>{device2.brand.raw} {device2.name.raw}</strong>
          {" "}
          uses{" "}
          {formatOS(device2)}. This affects the user interface experience and available software
          options on each device.
        </p>

        <p>
          Both devices are capable of emulating retro gaming systems, but their performance varies
          across different platforms. Review the detailed emulation performance specifications below
          to see how each device handles specific gaming systems.
        </p>
      </article>
    </details>
  );
